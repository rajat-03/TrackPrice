...
// these are variables which indicates 4 different types of email we can send
const Notification = {
    WELCOME: 'WELCOME',
...
export const generateEmailBody =  async (product: EmailProductInfo, type: NotificationType) => {
    ...
    // shorten the title of the product if it is too long
    const shortenedTitle = product.title.length > 20 ? `${product.title.substring(0, 20)}...` : product.title;

    let subject = ''
    let body = ''

    // switch statement is used to check the type of email we want to send
    switch (type) {
